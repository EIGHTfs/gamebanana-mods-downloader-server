#!/usr/bin/env bash
# ============================================================
# 用项目本地的 GitHub 推送凭据，推送到 origin/main
#
...
printf 'https://EIGHTfs:%s@github.com\n' "$TOKEN" > "$CRED_FILE"
chmod 600 "$CRED_FILE"
trap 'rm -f "$CRED_FILE"' EXIT

echo "▶ 推送 main → origin（使用项目本地凭据）..."
timeout 120 git -c credential.helper="store --file=$CRED_FILE" push origin main "$@"
echo "✅ 推送完成"
