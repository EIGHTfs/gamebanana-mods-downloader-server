#!/usr/bin/env bash
# ============================================================
# gbmd-v3 - macOS 启动脚本（与 start-linux.sh 同构）
# 用法：
#   重启（默认）：./start-macos.sh [restart] [--port 8642]
#   启动：        ./start-macos.sh start [--port 8642]
#   停止：        ./start-macos.sh stop
#   状态：        ./start-macos.sh status
#   设置密码：    ./start-macos.sh --set-password "新密码"
...
if [[ "$CMD" == "--port" ]]; then
  ...
  shift  # 去掉命令，剩余参数留给 restart/start
else
  # 未知参数，当做 restart 并保留所有参数（可能是旧用法）
  CMD="restart"
  # 不 shift，保留全部参数
fi
