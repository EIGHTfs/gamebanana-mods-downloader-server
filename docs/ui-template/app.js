// ============================================================
// GameBanana Mod Downloader - 前端主逻辑（tab + 轮询）
// ============================================================
"use strict";
...
// 纯前端模板：api() 由 docs/ui-template/mock-api.js 提供（返回空态 + 契约注释）。
// 接真实后端时：删掉下面 mock 说明、取消本块注释即可（真实实现样例保留如下）。
...
function loadGameSelects() {
  api("/api/games").then((r) => {
    ...
    // 2026-09-02 重建索引按游戏：下拉复用已配置游戏列表（值为游戏名）
    const hrg = $("#hashRebuildGame");
    if (hrg) {
      const saved = hrg.value;
      hrg.innerHTML = '<option value="">全部游戏</option>' +
        Object.entries(r.games).map(([name, entry]) =>
          `<option value="${esc(name)}">${esc(entry && entry.cn ? entry.cn + "（" + name + "）" : name)}</option>`).join("");
      hrg.value = saved;
    }
...
function makeRootRow(name, entry) {
  ...
  // 2026-09-02 目录选择统一走 path-picker.js 小模块：一行接入（自动加 📂 按钮 + 弹窗绑定）
  PathPicker.attach(div.querySelector(".root-path"));
  return div;
}
...
function bindSettings() {
  ...
  async function runScanIncomplete() {
    ...
    lastScanTaskJson = null;
    setStatus(st, "扫描全部游戏根目录（.part 残留 / 文件缺失）…", "");
    out.innerHTML = "";
    try {
  ...
  if (hr) hr.addEventListener("click", async () => {
    const st = $("#hashStatus");
    const gameSel = $("#hashRebuildGame");
    const game = gameSel ? gameSel.value : "";
    st.className = "status";
    st.textContent = game
      ? `后台重建「${game}」索引…（可继续查询旧索引）`
      : "后台重建全部游戏索引…（可继续查询旧索引）";
    try {
      ...
      const poll = async () => {
        ...
        const gInfo = (s && s.games && s.games[game]) || null;
        st.textContent = game
  ...
  api("/api/hash-index-status").then((s) => {
    if (s && s.ok) {
      const st = $("#hashStatus");
      if (st) {
        const gCount = s.games ? Object.keys(s.games).length : 0;
        st.textContent = "索引：GB 表 " + (s.gb || 0) + " 条 / 本地表 " + (s.local || 0) + " 条" +
          (gCount ? `（${gCount} 个游戏）` : "") + (s.running ? "，重建中…" : "");
        st.className = "status";
      }
    }
  }).catch(() => {});
...
// 2026-09-01 参照 iwara formatIwaraLoginBlock：检测登录状态多行块（含凭证明细）
